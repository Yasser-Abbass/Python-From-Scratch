...
def list_subscriber(page, per_page):
    subscriber = Subscriber.objects.order_by('first_name', 'last_name').paginate(page=page, per_page=per_page)
    return subscriber


def get_all_subscriber():
    subscriber = Subscriber.objects.order_by('email')
    return subscriber
...
def list_book(page, per_page):
    book = Book.objects.order_by('title').paginate(page=page, per_page=per_page)
    return book
...
def get_all_publishers():
    publisher = Publisher.objects.order_by('name')
    return publisher


def list_publisher(page, per_page):
    publisher = Publisher.objects.order_by('name').paginate(page=page, per_page=per_page)
    return publisher